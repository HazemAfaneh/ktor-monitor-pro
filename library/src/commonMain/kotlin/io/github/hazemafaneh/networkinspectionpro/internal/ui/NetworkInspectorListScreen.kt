    ...
    CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Ltr) {
        Scaffold(
            ...
            Column(
                ...
                Text(
                    text = "${uiState.logs.size} requests",
        ...
        if (showClearDialog) {
            AlertDialog(
                ...
                text = { Text("This will clear all captured network logs.") },
                confirmButton = {
                    TextButton(onClick = {
                        viewModel.clearLogs()
...
private fun NetworkLogCard(entry: NetworkLogEntry, onClick: () -> Unit) {
    val path = remember(entry.url) { extractUrlPath(entry.url) }
    val domain = remember(entry.url) { extractUrlDomain(entry.url) }
    val statusCode = entry.responseCode ?: 0
    val duration = entry.durationMs ?: 0L
    val durationColor = if (duration > 3000L) Color(0xFFE53935)
...
private fun EmptyNetworkView() {
    Column(
        ...
        Text(
            text = "API calls will appear here",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant
        )
    }
...
private fun extractUrlPath(url: String): String = try {
...
private fun extractUrlDomain(url: String): String = try {
    val withoutProtocol = if ("://" in url) url.substringAfter("://") else url
